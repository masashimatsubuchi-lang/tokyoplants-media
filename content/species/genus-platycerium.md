...
ビカクシダ属の約18種は、栽培難易度と入手しやすさの面で大きな差がある。初めてビカクシダを育てるなら、丈夫で耐寒性が高く、流通量も多いビフルカツムかヒリーを選ぶのが最善である。この2種は一般的な園芸店でも手に入り、多少の管理ミスにも耐える。

中級者向けとしては、コロナリウムやグランデなど貯水葉が大型化する迫力のある種が楽しめる。これらは温度管理と風通しへの注意が必要だが、正しく育てると見応えのある株になる。

リドレイとウィリンキーは美しさとレア感からコレクター人気が高いが、要求する環境も厳しく、高温・高湿度・十分な風通しの3つを同時に満たす管理が求められる。育成ライトと加湿器、サーキュレーターを組み合わせた「ビカクシダコーナー」を作ると、この2種の真価が発揮される。
...
板付けはビカクシダ栽培の醍醐味であり、着生植物本来の生育環境を再現できる最適な仕立て方である。

**用意するもの：** 木板（コルク板・杉板・ヘゴ板など）、水苔（十分に水で戻したもの）、テグスまたは木綿糸、フック（壁に掛けるため）。
