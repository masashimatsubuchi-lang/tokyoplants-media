...
1. <mark>**フロリダビューティー最大の個性は「多裂葉×斑入り」の組み合わせ——フロリダ（*squamiferum* × *pedatum*）の複雑に裂けた葉形にクリーム色の斑が散るビジュアルは、シンプルな斑入り種とは次元の異なる存在感を生む。** 斑の出方は株ごと・葉ごとに異なり、全く同じ株は二度と作れない。</mark>
2. **斑の安定には「明るい間接光」が最重要——光不足が斑の退行（全緑化）の最大の原因。** 葉緑素が少ない斑部は光合成効率が低く、暗い環境では緑の葉優先で生存しようとするため、明るい環境の維持が斑を保つ唯一の手段。
3. **支柱誘引で葉のサイズと裂け方が増す——上に伸びるほど裂け目が深くなり、斑のコントラストも強調される。** 横這いで管理すると葉が小型化し、フロリダビューティーらしい表情が出にくくなる。
...
| 斑のパターン | 見た目の特徴 | 株への負担 | 安定性・希少度 |
|---|---|---|---|
| セクター斑 | 葉の一部が大きくクリーム〜白色に染まる | やや大きい（光合成面積が減る） | コントラスト重視の中間タイプ |
| スプラッシュ斑 | 細かい点・飛び散り状の斑が散る | 少ない | 最も安定しやすい |
| ハーフムーン | 葉の左右どちらかが完全に白〜クリーム | 大きい | 最も希少で市場価格も高い |

斑のパターンは遺伝的に固定されておらず、同じ株から展開する葉であってもパターンが変化することがあります。

<!-- character-note character="bloom" type="tip" -->
斑の出方は葉ごとに変わるから、「前の葉と比べてどう変化したか」を記録しておくと面白いよ。育成ログに展開した葉の写真を残していくと、株ごとの斑の個性が時系列で見えてくる。
<!-- /character-note -->
