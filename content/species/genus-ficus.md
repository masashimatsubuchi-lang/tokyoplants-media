...
フィカス属の大きな魅力のひとつが、剪定と仕立てによる樹形の自由度である。同じ品種でも育て方次第でナチュラルな一本幹仕立て、複数幹が広がるブッシュ型、コンパクトに整えた丸みのある樹形など、全く異なる印象に仕立てられる。

**剪定による分岐の促し：** フィカスは剪定した位置の直下の節から新芽が出て分岐する。1本の幹が縦に伸びすぎた場合、好みの高さで切り戻すと2〜3本の枝が出て横に広がりのある樹形になる。この操作を数回繰り返すと、自然とボリュームのある株になる。

**幹の曲げ仕立て：** ウンベラータやゴムの木など、幹が若い段階で柔軟な種は、ワイヤーで固定することで幹を好みの角度に曲げる仕立てができる。曲げた状態を数ヶ月間維持すると、その形のまま木質化して固定される。S字やらせん状に仕立てた株はインテリア性が高く人気がある。

**剪定の注意点：** 剪定は必ず成長期（5〜9月）に行う。冬季の剪定は回復が遅く、切り口からの樹液が止まりにくくなることがある。切り口には殺菌を兼ねて木酢液や市販の癒合剤を塗ると、その後の管理がしやすくなる。

---

## 種ごとの置き場所の選び方

フィカス属は種によって光の要求量と環境変化への耐性が異なるため、置き場所の選定が長期管理の鍵になる。

**光量が多く必要な種：** ゴムの木（エラスティカ）・アルテシマ・ガジュマルは光を好み、明るい窓辺や屋外の半日陰でよく育つ。斑入り品種は特に光量不足で斑が消えやすいため、できるだけ明るい場所を確保する。

**中程度の光で管理できる種：** ウンベラータ・ベンガレンシスは明るい間接光で十分育つ。直射日光が当たると葉焼けしやすいため、レースカーテン越しの光が理想的。

**環境を固定すべき種：** ベンジャミンとリラータは特に置き場所の変更に敏感で、頻繁に動かすと落葉を繰り返す。「ここ」と決めたら動かさないことが管理の基本。エアコンの風が直接当たらない場所を選ぶことも重要。
