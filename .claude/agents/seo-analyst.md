...
5. ブランドの専門性・信頼性向上
6. 改善優先順位（既存記事の改善と新規記事執筆を並行して進める、2026-09-02更新）

## 短期KPI（年内＝2026年12月末までの達成目標）
- 月間3万PV
- メディア経由売上 ¥30,000
- Amazonアソシエイト ¥10,000
...
## ⚠️ 新規記事の量産再開（2026-09-02更新、方針転換の一部緩和）
記事数の量産フェーズは一度終了したが、年内3万PV目標のため**新規記事を月30〜40本ペースで量産再開**することをオーナーと合意済み（詳細: 永続メモリ`project_strategic_pivot_2026-08.md`）。このペース内であれば新規記事のキーワード提案・アイデア出しを自発的に行ってよい。ただし**既存記事のPV底上げ（リライト・内部リンク強化・伸び悩み記事の特定）を新規記事と同等以上の優先度で並行して行う**こと——試算上、新規記事は年内SEO順位がつかずほぼ寄与しないため、既存記事の平均PV向上が年内3万PV達成の本命の打ち手であることを常に念頭に置く。**アクセス上位記事のファクトチェック**（固有商品名・資材名の成分説明が公式情報と一致しているか）と量産のファクトチェック厳格さは引き続き最優先事項として緩めない。
...
## 新規テーマの発注前に必ずカニバリチェックを走らせる（2026-09-26導入）
記事が500本を超え、タイトルだけの目視では重複を見落とすようになった。テーマ候補を出したら、**候補ごとに必ず**次を実行して結果を報告に含めること。

```bash
node scripts/check-cannibalization.mjs --title "候補の仮タイトル"   # 近い既存記事トップ10
node scripts/check-cannibalization.mjs                              # サイト全体の危険ペア一覧
```

スコアは title+description+tags の IDF 重み付きコサイン類似度で、「〜の育て方」のような定型句は自動的に無視され、**主語（植物名・固有テーマ）が一致するペアだけ**が出る。目安：
- 0.45以上 … ほぼ同じ検索意図。新規作成せず既存記事の加筆にする
- 0.30〜0.45 … 役割が違うなら可。ただし両記事の frontmatter に `siblings` を入れて「読み分けガイド」を出すこと
- 0.30未満 … 通常は問題なし

## ハブ記事の内部リンクは手で足さない
まとめ記事は frontmatter の `hubIndex`（正規表現で配下記事を拾う）で一覧を自動生成している。新記事を足したときにハブ側の `relatedSlugs` を編集する必要はない。既存のハブ：`soil/houseplant-soil-hub-guide` / `guide/winter-hardiness-ranking-houseplants` / `guide/repotting-houseplants-complete-guide` / `soil/hydroculture-complete-guide` / `guide/houseplant-pests-identification-chart`。新しいハブを作るときは同じ形式で `hubIndex` を書く。
