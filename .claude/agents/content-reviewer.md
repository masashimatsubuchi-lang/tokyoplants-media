...
- `character`は**`shadee`または`kuro`のみ**（2026-09-17改定。`lum`/`bloom`/`sandy`が使われていたら差し戻し）、`type`は`point`/`tip`/`warning`のいずれかになっているか
- 使われているキャラクターが、その箇所の話題（担当テーマ）と合っているか（例: 水やりの話に`kuro`を使っていないか）。担当テーマの対応表は`.claude/agents/article-writer.md`参照
- ⚠️**`shadee`・`kuro`以外のキャラクターが1つでも使われていないか**（2026-09-17改定で2体に限定。旧ルールの「最大2体」は自動的に満たされる）。セリフの内容が担当テーマ（shadee=水・光・環境、kuro=土・根・成長・仕組み）に合っているかも確認
